...
|---|---|
| 深度（D） | 1 |
| 损失权重调度 | MTP loss weight λ = 0.3 for the first 15T tokens, then 0.1 for the remaining tokens (paper §2.4). |

_共享模块：_ 1 MTP layer (paper Table 1; config num_nextn_predict_layers=1) added as an MoE layer, shares the main model's vocabulary. Paper §2.1: 'we add an MoE layer as the MTP (Multi-Token Prediction) layer to support speculative decoding during inference'. Inference: speculative decoding via `--speculative-config.method mtp --speculative-config.num_speculative_tokens 1` (vLLM) or EAGLE 3-step (SGLang).

**Fill-in-Middle (FIM)：**

| | |
|---|---|
| 格式 | [Unknown/Not Disclosed] |
| 比例 | All source code data — paper §2.2: 'the Fill-In-the-Middle training objective is applied to all source code data'. |
...
_由 `data/extracted/glm-4.7.json` 通过 `python -m llm_tech_matrix.extraction.render` 自动生成。请勿直接编辑此文件——修改 JSON 或渲染器。_
