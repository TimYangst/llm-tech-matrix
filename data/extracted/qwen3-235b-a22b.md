...
**Routing:** Fine-grained expert segmentation (Dai et al., 2024 - DeepSeekMoE) over 128 experts with top-8 routing per token (norm_topk_prob=true). Unlike Qwen2.5-MoE and DeepSeek-V3, Qwen3-MoE excludes shared experts. Load balancing uses the global-batch load balancing loss (Qiu et al., 2025) with router_aux_loss_coef=0.001 - balancing is computed across the entire global batch rather than per-sequence to encourage richer expert specialization.
...
**Data mix notes:** Roughly 2x the pre-training tokens of Qwen2.5 with 3x more languages (29 -> 119). High-quality content across coding, STEM, reasoning, books, multilingual texts, and synthetic data. Synthetic tokens generated by Qwen2.5, Qwen2.5-Math, and Qwen2.5-Coder. PDF-derived tokens recovered via Qwen2.5-VL OCR + Qwen2.5 refinement. A multilingual data annotation system tags >30T tokens by educational value, fields, domains, and safety; data mixture is optimized at the instance level via ablations on small proxy models. The Qwen3 paper §3.1 implies that dense and MoE flagships share the same pre-training corpus and recipe; per-token cost is much lower for MoE due to 22B/235B activation ratio. No quantitative percentage breakdown disclosed.
