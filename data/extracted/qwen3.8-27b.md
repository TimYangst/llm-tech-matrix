...
> 中文版：[qwen3.8-27b.zh.md](./qwen3.8-27b.zh.md)
...
_Shared modules:_ MTP head with mtp_num_hidden_layers=1 (config) and mtp_use_dedicated_embeddings=false (shares input embeddings with the main model). README states 'MTP: trained with multi-steps' - exact step depth D is not disclosed. Unchanged from Qwen3.6-27B.
...
**Applies when:** Thinking mode only — with enable_thinking=false the template skips effort resolution entirely.
...
**Stability tricks:** [Unknown/Not Disclosed]
...
**Fusion notes:** Same native-early fusion as Qwen3.5-27B / Qwen3.6-27B. Vision tokens are inlined into the same backbone as text via four reserved vocabulary IDs (vision_start, image, video, vision_end). vision_config.out_hidden_size=5120 matches LM hidden_size. HF pipeline_tag=image-text-to-text. README highlight: 'Native support for image and video understanding, from STEM diagrams and documents to hour-scale videos.'

### Vision encoder

| | |
|---|---|
| Architecture | ViT (HF vision_config model_type=qwen3_5 — still reporting the Qwen3.5 vision model type three generations on, confirming a shared vision encoder). config.vision_config.deepstack_visual_indexes=[] - no DeepStack injection layers configured. |
| Depth (layers) | 27 |
| Hidden size | 1152 |
...
_Notes:_ hidden_act=gelu_pytorch_tanh; num_position_embeddings=2304. Identical geometry to Qwen3.5-27B and Qwen3.6-27B, and preprocessor_config.json is byte-identical across all three (sha256 27225450ac9c...) — the image preprocessing pipeline has not changed since Qwen3.5.
...
_Generated from `data/extracted/qwen3.8-27b.json` by `python -m llm_tech_matrix.extraction.render`. Edit the JSON, not this file._
