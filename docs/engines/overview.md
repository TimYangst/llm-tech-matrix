...
- **Registry: no engine slots (decided in E4).** Registry slots exist to map free-text or
  enum values in model records onto slugs. `technique_support[].glossary_slug` already *is*
  a slug, and `scripts/validate_extractions.py` rejects unknown ones, so engine→technique
  edges are read directly. The "Techniques × engines" table in the support matrix sets both
  sides next to each other.
- **First synthesis outputs (E4)**, generated by `synthesis/engines.py` via
  `python -m llm_tech_matrix.synthesis.index` and checked for staleness in CI:
  - [`data/reports/engine-support-matrix.md`](../../data/reports/engine-support-matrix.md):
    models × snapshots, techniques × snapshots, per-model details, integration version
    constraints.
  - [`data/reports/engine-adoption.md`](../../data/reports/engine-adoption.md): how soon
    after release each engine carries each model. Model dates are month-precision, so the
    lag is in months, a same-month `not found` is not counted as missing, and support
    without a recorded `since_version` is only an upper bound.
  - "Implemented by (engines)" sections in glossary entries, EN and ZH, between generated
    markers.
