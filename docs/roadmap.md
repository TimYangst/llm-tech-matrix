...
- **TODO: `add-model-source` skill** — wrap the "create manifest entry + fetch + record sha256" workflow currently exposed as `python -m llm_tech_matrix.sourcing add ...`. Defer until we've onboarded several models manually and seen what the friction points actually are.
