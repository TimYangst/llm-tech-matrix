...
| DeepSeek-V4-Pro   | 用在 V4 两阶段后训练的按领域 Specialist Cultivation RL 阶段（specialist -> 多教师 OPD）。超参与 DeepSeek-V3.2 紧密对齐。易验证任务用规则验证器 / unit test；难验证任务用 rubric-guided RL 数据 + Generative Reward Model (GRM) 评估——actor 网络兼任 GRM，判别能力与生成能力联合优化。对每个推理力度（Non-think / Think High / Think Max）使用不同的长度惩罚与上下文窗口。                                                                                                                                                                                                                                                                                                                                                                                |
| DeepSeek-V4-Flash | GRPO 配方与 V4-Pro 完全相同（同一 V4 系列流水线）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| GLM-5 / GLM-5.1   | GRPO + **IcePop** 修改（论文 §3.2）——pop-style mask 把训练-推理 policy 不匹配率落在 `[1/β, β]` 之外的 token 屏蔽掉，明确区分梯度更新用的 `π_train` 和轨迹采样用的 `π_infer`。相比原始 IcePop，GLM-5 **去掉了 KL 正则项**以加速 RL 提升。Reasoning RL 超参：β=2，ε_low=0.2，ε_high=0.28，group size 32，batch size 32。Agentic RL（论文 §3.3 / §4.1.2）通过 Direct Double-sided Importance Sampling 完全抛弃 `π_old` 跟踪（直接用 rollout 的 log-probabilities，加 token 级 clipping `[1−ε_l, 1+ε_h]`）；最后的 On-Policy Cross-Stage Distillation 阶段 group size 1（advantage 来自 teacher gap，不需要方差估计）。DSA 在 RL 期间的稳定性要求 indexer 的 top-k 必须 deterministic（用 `torch.topk`），且默认冻结 indexer 参数（见 [DSA](./dsa.zh.md)）。 |
