...
| DeepSeek-V4-Pro   | 沿用 DeepSeek-V3 的 PSM 格式（论文第 4.1 节）。比例未重述——除非 V4 后续放出消融，否则按 V3 的 0.1 处理。                                                                                                                                                                                                                                                                     |
| DeepSeek-V4-Flash | 与 V4-Pro 相同的 PSM 继承。比例未重述。                                                                                                                                                                                                                                                                                                                                      |
| GLM-4.7           | "the Fill-In-the-Middle training objective is applied to all source code data"（GLM-4.5 ARC paper §2.2）。FIM 应用于整个代码语料而非按比例随机——与 DSV3 的 10% PSM 比例不同。具体 token 格式（PSM vs SPM）和特殊 token 未披露。GLM-5 继承相同代码预训练 pipeline（GLM-5 paper §2.2 "Building upon the GLM-4.5 data pipeline"），所以 FIM 推测沿用，但 GLM-5 论文未单独确认。 |
