...
| GLM-5.3-Flash | **First KDA deployment outside Moonshot.** `linear_attn_config` = 64 heads, head_dim 128, short causal conv 4, `gate_lower_bound=-5.0` (a floor on the decay gate, bounding how fast the recurrent state can forget). The layer list in the config is literally named `kda_layers`: 34 of 45 layers, interleaved 3:1 with DSA-MLA layers. Z.AI pairs it with **NoPE** MLA, the same combination Kimi K3 uses — the linear layers carry position, so the softmax layers don't need rotary.                                                                                                       |
