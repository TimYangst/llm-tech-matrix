...
| Qwen3.8-27B        | Identical mRoPE configuration to Qwen3.5/3.6-27B — `mrope_section=[11,11,10]`, `mrope_interleaved=true`, `partial_rotary_factor=0.25`, base 10,000,000. Note the contrast within the same generation: the text-only **Qwen3.8-2.4T-A95B drops mRoPE entirely** (no `mrope_section` / `mrope_interleaved`) and uses plain partial RoPE, since there is no vision axis to partition.                                                               |
| Qwen3.8-Flash-Next | Unchanged mRoPE configuration (`mrope_section=[11,11,10]`, interleaved, partial 0.25, base 1e7) — but now with a documented justification for keeping RoPE at all. Report §2.1.1 tested a **NoPE** variant and rejected it: indistinguishable from RoPE during pre-training, but 'a substantially higher rate of endless generation after post-training'. A direct counterpoint to Kimi K3, which shipped NoPE.                                  |
