...
| Kimi K3                | **Multi-Teacher On-Policy Distillation (MOPD)** — the same family as DeepSeek-V4's OPD, but multi-teacher along **two axes** rather than one: 3 RL domains (general / general agents / coding agents) x 3 reasoning-effort levels {low, high, max} = 9 expert teachers, consolidated into the single released checkpoint. For a sampled domain d and effort e, the per-token reward is `clip(sg(log π_teacher(y_t) − log π_student(y_t)), −R_max, R_max)`. Because it is a dense reward inside the same RL framework, it inherits infrastructure optimizations like partial-rollout training for long-horizon tasks. Finer-grained top-k distillation objectives were tried and showed no clear advantage in convergence speed or final performance. |
| DeepSeek-V4-Flash-0731 | Inherits the preview's >10-teacher OPD; whether the 0731 re-post-training repeats it with the same teacher set is undisclosed. Separately, the attached DSpark draft is itself distilled from the frozen target's output distributions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
