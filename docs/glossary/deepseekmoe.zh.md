...
| Kimi K2 家族（K2-Thinking、K2.5、K2.6） | **384 路由 × 1 共享，top-8**，单专家中间维度 2048（稀疏度 48——K2.5 论文 §4.1）。61 层中第 1 层为 dense（intermediate_size=18432），其余 60 层为 MoE。与 DeepSeek-V3 相同的"细粒度专家 + 1 个共享专家"模板，单专家宽度也相同（2048），但专家数多 1.5 倍（384 vs 256），且 dense 前缀只有 1 层（V3 是 3 层）。配合 Sigmoid 的 aux-loss-free routing（无专家分组）。MoE 专家权重在 K2-Thinking + K2.5 + K2.6 上经 [native INT4 QAT](./int4-qat.zh.md) 后部署为 INT4。                                                  |
| GLM-4.7                                 | **160 路由 × 1 共享，top-8**，单专家中间维度 1536（稀疏度 20）。92 层中前 3 层 dense（intermediate_size=12288），其余 89 层 MoE。GLM-4.5 ARC paper §2.1 明确对比 GLM-4.5 系列与 DeepSeek-V3 / Kimi K2 的设计哲学：减小宽度（hidden_dim 和路由专家数）、增加高度（层数），因为更深的模型在推理任务上表现更好。96 个注意力头 / hidden 5120 = 比 DSV3 高 2.5 倍的 heads-per-hidden 比例，原因是 reasoning benchmark 上有收益。                                                                                         |
| GLM-5 / GLM-5.1                         | **256 路由 × 1 共享，top-8**，单专家中间维度 2048（稀疏度 32）。78 层中前 3 层 dense（intermediate_size=12288），其余 75 层 MoE。GLM-5 论文 §2.1 又把 GLM-4.5 的"更深更窄"哲学翻转回来："scales to 256 experts and reduces its layer count to 80 to minimize expert parallelism communication overhead"。"细粒度专家 + 1 个共享"模板与 DeepSeek-V3 相同，单专家宽度也相同（2048），但 78 层 vs DSV3 的 61 层（仍更深），并组合 [DSA](./dsa.zh.md) 稀疏注意力 + [MLA](./mla.zh.md) 潜变量 KV，而不是 DSV3 的纯 MLA。 |
| Kimi K3                                 | 保留共享+路由的组织形式，但改变了路由专家读入的内容：它们在 3584 宽的**潜空间**中运算（隐藏维 7168 的 0.5 倍），而非全模型宽度 —— 见 [Stable LatentMoE](./latentmoe.zh.md)。896 路由 / 激活 16（稀疏度 56）+ 2 个全宽共享专家，单专家 FFN 宽度 3072。1 层稠密后接 92 层 MoE。Sigmoid router、`noaux_tc`、`routed_scaling_factor=1.0`，无分组与 node-limited 路由。                                                                                                                                                  |
| DeepSeek-V3.2-Exp                       | MoE 部分与 DeepSeek-V3 逐字节相同：256 路由 / 激活 8 + 1 共享，单专家宽度 2048，61 层中前 3 层为稠密，`routed_scaling_factor=2.5`，并保留 node-limited 路由（`n_group=8`、`topk_group=4`）—— 这一约束在 V4 世代被去掉。                                                                                                                                                                                                                                                                                             |
| DeepSeek-V4.1-Flash                     | 384 个路由专家 top-6 + 1 个共享专家，单专家宽度 2,304（V4-Flash：256 / 2,048）。**全部 40 层**都是 MoE FFN；V4-Flash 前 3 层的哈希路由看起来已被去掉（config 中没有 `num_hash_layers`）。新增文本 / 图像分模态的 aux-loss-free 偏置。DSpark drafter 自带一个更小的 MoE（128 个路由专家，top-3）。                                                                                                                                                                                                                   |
