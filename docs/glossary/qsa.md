...
| GLM-5.3-Flash (convergent) | Not QSA by name, but the same mechanism arrived at independently within days: `index_kpool=4` + `index_kpool_compress=true` pools indexer keys 4:1 before scoring, and `index_kpool_always_select_tail=true` forces the trailing incomplete block to be selected — matching QSA's `r=4` micro-blocks and always-included tail. Notably GLM reached it by *abandoning* the cross-layer alternative it had shipped two months earlier ([IndexShare](./indexshare.md)) on moving to a hybrid stack — which is exactly the argument Qwen's report makes. |
