...
| Kimi K3   | **MoonViT-V2** —— 27 层、约 401M 参数，hidden 1024 / intermediate 4096 / 12 头 / patch 14，经 `patchmergerv2` MLP projector 投影到 LM 隐藏维 7168，并做 2×2 pixel-shuffle 下采样（视觉 token 减少 4 倍，使最高 3584×3584 像素的输入在 1M 上下文内仍可承受）。相对 MoonViT-3D 的关键变化：**完全从零开始用 next-token prediction 训练**，而非从 SigLIP 初始化。动机是训练稳定性 —— SigLIP 初始化的编码器视觉塔梯度范数持续偏高且频繁尖刺，而 MoonViT-V2 全程稳定（论文 Fig. 6）—— 同时也让语言建模目标直接塑造视觉表示，而不是让偏好全局语义、忽略细粒度文字与结构线索的对比损失来塑造。它在视觉评测上与 SigLIP 初始化基线持平，论文据此认为大规模下对比预训练作为初始化并非必需。此外它使用 RMSNorm，并去掉线性层与注意力投影中的全部 bias。图像与视频完全共享参数，注意力被分解为帧内空间与帧间时序两轮，并辅以时序池化。 |
