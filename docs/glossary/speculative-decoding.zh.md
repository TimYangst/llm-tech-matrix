...
| DeepSeek-V4.1-Flash    | DSpark 仍随 checkpoint 发布，并且 draft 现在带 MoE：三个 128 token SWA 的 Transformer block，128 个路由专家 top-3（`dspark_n_routed_experts`、`dspark_num_experts_per_tok`），每次前向产出五个 draft 位置，Markov head 秩 256，以主干最后三层 `[37, 38, 39]` 为条件。**主干预训练不再使用 MTP 目标。** DSpark 在预训练之后有专门的一个阶段、冻结主干单独训练，随后在后训练中继续训练但不向主干回传梯度，因此也能加速 RL 和 OPD 的 rollout。`num_nextn_predict_layers=3` 与三个 draft block 对应。本次发布没有公布推理服务的启用参数。             |
