...
| Qwen3.8-Flash-Next     | 仍是 `mtp_num_hidden_layers=1` / `mtp_use_dedicated_embeddings=false`，但 MTP 头现在本身就是一个**结构化模块**：config 里带有 `mtp` 子对象（`hybrid: true`、自己的 `layer_types`、自己的 `rope_theta`），在参数预算中按约 4B 单独统计，其注意力层本身也被替换为 [QSA](./qsa.zh.md)。仿照 GLM-5，它**在投机解码的各步之间复用 QSA 的 top-k 索引**——报告 Tab. 4 显示这是免费的：四步投机解码下平均接受长度全注意力 4.06、QSA 4.07。训练时的步数 D 仍未披露。                                        |
| GLM-5.2                | `num_nextn_predict_layers=1`，结构未变，但**为投机解码做了改进：接受长度最多 +20%**（模型卡原话，未给方法）。`index_share_for_mtp_iteration=true` 让 MTP 各步复用骨干的 DSA 选择而不必重算——这个索引复用技巧后来被 Qwen3.8-Flash-Next 记在 GLM 名下。                                                                                                                                                                                                                                             |
| GLM-5.3-Flash          | `num_nextn_predict_layers=1`，与 GLM-5 线一样参数共享；`index_share_for_mtp_iteration=true` 保留。                                                                                                                                                                                                                                                                                                                                                                                                |
