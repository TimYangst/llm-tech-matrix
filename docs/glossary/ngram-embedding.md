...
| DeepSeek-V4.1-Flash | **Engram**, DeepSeek's own design (Cheng et al., 2026, arXiv:2601.07372). **196B** parameters in **two** modules at layers **1 and 14** (placed to balance memory across pipeline stages) — versus Qwen's single layer. Each module: n-gram orders {2, 3, 4}, 8 hash heads, total dim 2,048 per order, each head a ≈16M-entry table with distinct-prime sizes; tokenizer compression 129,280 → 99,092 ids; context-aware gating. The short causal convolution is **removed**. Tables and K/V projections in FP8. Tables trained with a Sinkhorn-balanced momentum update instead of Adam, Engram LR 5×. Inference prefetches from host memory via background RDMA, overlapping the first block; during RL rollouts the tables stay in GPU memory. |
