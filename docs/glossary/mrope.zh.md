...
| Qwen3.8-Flash-Next | mRoPE 配置未变（`mrope_section=[11,11,10]`、interleaved、partial 0.25、base 1e7）——但这次有了「为什么还留着 RoPE」的公开依据。报告 §2.1.1 测试了 **NoPE** 变体并否决了它：预训练期间与 RoPE 无法区分，但「后训练之后出现无休止生成的比例显著更高」。这与上了 NoPE 的 Kimi K3 形成直接对照。                                                                                     |
