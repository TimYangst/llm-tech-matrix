...
| GLM-5.3-Flash（殊途同归） | 名字不叫 QSA，但机制在数日之内被独立地做到了同一处：`index_kpool=4` + `index_kpool_compress=true` 在打分前把 indexer 的 key 按 4:1 池化，`index_kpool_always_select_tail=true` 强制选中末尾不完整的块——与 QSA 的 `r=4` 微块和「尾块始终包含」完全对应。值得注意的是，GLM 是在转向混合栈后*放弃*了自己两个月前发布的跨层方案（[IndexShare](./indexshare.zh.md)）才走到这里的——而这正是 Qwen 报告里给出的论证。                                                                |
