...
| Kimi K3                | **首个采用 MXFP4 的非 DeepSeek 厂商**，对 Moonshot 也是一次格式切换：K2 家族（K2-Thinking / K2.5 / K2.6）发布的是原生 INT4，K3 改为 MXFP4 权重 + **MXFP8 激活**，理由是「更广的硬件兼容性」。QAT 从 SFT 阶段一直贯穿*整个* RL，且 rollout 与训练共用同一量化方案，因此不存在训练-推理失配 —— 这比 DeepSeek-V4 的后训练 QAT 更强。范围仅限 MoE 专家权重；`config.quantization_config.ignore` 排除了 self_attn、shared_experts、稠密 mlp 投影、lm_head、vision_tower 与 mm_projector，潜空间 MoE 投影与 router 同样保持较高精度。格式：compressed-tensors `mxfp4-pack-quantized`、num_bits=4、type=float、group_size=32、对称。EAGLE-3 草稿模型也在同一配置下微调。 |
