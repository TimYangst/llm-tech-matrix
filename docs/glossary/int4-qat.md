...
| Kimi K3          | **Does not use INT4** — listed here because it marks the end of the recipe's run at Moonshot. K3 switches to MXFP4 weights + MXFP8 activations "for broad hardware compatibility", and moves QAT earlier: from SFT onward through all of RL, with rollout and training sharing the scheme. The scope stays the same (routed expert weights only, same `ignore` pattern shape), so the delta is purely the numeric format. See [FP4 QAT (MXFP4)](./fp4-qat.md). |
