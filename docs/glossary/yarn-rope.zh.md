...
| Qwen3.8-Flash-Next  | 与家族配方一致——在 `original_max_position_embeddings=262144` 之上 `factor=4.0` → 1,000,000，卡片里重新给出了完整的「Processing Ultra-Long Texts」段落和各框架的 override 片段（2.4T-A95B 的卡片省略了这部分）。注意这里的原生窗口是真正*训练过*的、不是外推：继续预训练就跑在 256K。                                                                                                                                                                                                      |
| DeepSeek-V4.1-Flash | YaRN factor 16，`original_max_position_embeddings=65536`，`beta_fast=32`，`beta_slow=1`，rope_theta 10,000，压缩分支 `compress_rope_theta=160000`——数值上与 V4-Flash 完全相同（键名由 `type` 改为 `rope_type`）。长度课程比 V4 简单：稀疏注意力在 64K 下从零训练，在 45T token 中的 **34T 处扩展到 1M**。部分 RoPE 作用于 512 维中的 64 维。                                                                                                                                              |
