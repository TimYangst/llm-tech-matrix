...
| Kimi K3                                 | **Per-Head Muon**（论文 §2.5）。对注意力投影，不再对完整的 Q/K/V 投影矩阵做 Newton-Schulz 正交化，而是沿头维切分动量矩阵、对每个头的块分别正交化。理由：整矩阵正交化把所有头当作一个耦合块处理，于是梯度/动量尺度大的头会主导共享的更新方向，尺度小的头则得不到充分归一化的更新。逐头正交化让各头更新尺度均衡，*而且*略微更省 —— 在瘦高的逐头块上做 Newton-Schulz 比在完整投影矩阵上便宜。搭配 Kimi K2 的权重裁剪机制；weight decay 0.1。分布式实现采用基于 P2P 的正交化 —— 每个 rank 只按需拉取自己所属参数的分片，而不是 all-gather 整个参数缓冲区。                                                                                                                                                                                                                                                                                                                      |
| Qwen3.8-Flash-Next                      | **Qwen 的第一个 Muon 模型**，至此集齐——DeepSeek-V4、Kimi 线、GLM，现在加上 Qwen。值得注意的是参数分类切得很细：Muon 覆盖注意力 q/k/v/o、GDN 输入输出投影、专家 fc1/fc2 以及 n-gram 的 k/v 投影；**AdamW 保留**嵌入层、输出头、MoE 路由（Muon 会加剧早期波动且后期无收益——路由的每个输出维度是彼此独立的专家打分，没有可供正交化利用的共享线性结构）、GR 的低秩投影（形状过于狭长）、GDN 的 decay/beta（是向量不是矩阵）以及各输出门。**融合参数在正交化前先拆开**——qkv 和 GDN 输入按 head 粒度拆、SwiGLU fc1 拆成 gate/up——因为对拼接矩阵做正交化会把互不相关子块的奇异方向混在一起。Newton–Schulz 取 8 步、用 Polar Express 系数调度，选它是为了稳定性而非精度。两个工程后果：`Canzona` 按估计的 NS FLOPs 重新均衡 DP 分区（开销对较短维度是三次方的，等元素数切分会造成严重掉队），以及整个 optimizer step 被 CUDA graph 捕获，因为拆分后每层会产生约 100 个极小 kernel。 |
