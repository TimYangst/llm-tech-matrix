...
- **DeepSeek-V4.1** drops named prose levels for a *continuous integer* (1–100) rendered as a
  `<｜System｜>` prefix ("Reasoning Effort: 75 (range 1-100, …)"); `low` / `high` / `max`
  survive only as aliases for 50 / 75 / 100.

None of the four is a control token, a routing decision, or a separate set of weights.
...
prompt engineering with a vendor-blessed wording? Two vendors have now published training
recipes. Kimi K3 trains separate effort-level experts under per-problem token-budget rewards
and distills them back into one checkpoint; DeepSeek-V4.1 trains a single policy, sampling
several effort levels per prompt, computing advantages within each (prompt, effort)
subgroup, and applying a length penalty whose coefficient decays exponentially with effort —
which is also why it can interpolate between trained levels. The other implementations
remain undocumented.
...
- Reference implementation: chat templates in the model repos — `tokenizer_config.json`
  for Qwen3.8 / Kimi K3; `encoding/README.md` for DeepSeek-V4-Flash-0731 and DeepSeek-V4.1-Flash.
- Relevant blog/post: —
...
| DeepSeek-V4.1-Flash    | **Fourth mechanism: a continuous integer scalar.** `reasoning_effort` ∈ [1, 100], rendered once at conversation start as a `<｜System｜>` block — `Reasoning Effort: {budget} (range 1-100, the higher the value, the more thorough the reasoning)`. Aliases `low` / `high` / `max` = 50 / 75 / 100; default **`high`** (0731's default `low` carried no prefix). The training recipe is documented: responses are sampled at several effort levels per prompt, advantages are mean-centered within each (prompt, effort) subgroup, and a length penalty's coefficient decays exponentially with effort, `k(b) = k0·exp(−(b − b_min)/τ)`. Trained on a finite set of levels; intermediate values interpolate. Effort 25 → 100 lifts DeepSWE v1.1 66.0 → 74.2 at ≈2.5× output tokens. |
