...
| DeepSeek-V4-Pro   | Per-head RMSNorm on each query head and on the single shared KV head before core attention (paper §2.3.3). Explicitly noted as the replacement for the QK-Clip technique.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| DeepSeek-V4-Flash | Identical per-head Q/KV RMSNorm recipe as V4-Pro.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| GLM-4.7           | RMSNorm-style QK-Norm in every attention block (`config.use_qk_norm=true`). GLM-4.5 ARC paper §2.1: "We also incorporate QK-Norm to stabilize the range of attention logits." Among the four models compared in paper Table 1 (GLM-4.5, GLM-4.5-Air, DeepSeek-V3, Kimi K2), only GLM-4.5 has QK-Norm=Yes — the other three rely on different stability tricks (DSV3: none disclosed; K2: MuonClip QK-Clip). Note: GLM-4.7 uses both QK-Norm *and* `attention_bias=true`, distinguishing it from Qwen3 (QK-Norm only, no QKV bias). GLM-5 then drops QK-Norm because Muon Split already keeps logits stable (see [Muon](./muon.md)). |
