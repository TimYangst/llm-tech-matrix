...
| GLM-4.7            | 96 个 query head，8 个 KV head（group size 12——比 Qwen3-32B 的 8:1 更激进，但不如 Qwen3-235B-A22B 的 16:1），`head_dim=128`。配合 **QK-Norm**（论文 §2.1）和 `attention_bias=true`（注意 Qwen3 家族用 `attention_bias=false`）。**Partial RoPE 仅作用于一半 head_dim**（`partial_rotary_factor=0.5`）。论文 §2.1 明确解释更宽 head 的设计（96 head / hidden 5120 = 比 DSV3 高 2.5 倍 heads-per-hidden）："consistently improves performance on reasoning benchmarks such as MMLU and BBH"，虽然 training loss 没下降但 reasoning benchmark 收益明显。 |
| Qwen3.8-27B        | 与 Qwen3.6-27B 完全相同——24Q、4KV、head_dim 256、输出门控、partial RoPE 0.25、64 层中 16 层。`config.json` 除 `transformers_version` 外逐字节一致。                                                                                                                                                                                                                                                                                                                                                                                                   |
| Qwen3.8-2.4T-A95B  | 64Q、4KV → **16:1 共享比例**，是 Qwen 3.x 线里最激进的（27B 为 6:1）。head_dim 256 与输出门控不变。KV 宽度 4×256=1024——**与 27B 完全相同**，尽管 hidden 大了 1.6 倍，因此每层 KV-cache 成本不随规模增长。92 层中 23 层。                                                                                                                                                                                                                                                                                                                              |
| Qwen3.8-Flash-Next | 24Q / **2KV**——12:1 的比例，把 Qwen3.8-27B 的 6:1 又翻了一倍，KV 宽度减半到 2×256=512。head_dim 256 与 partial RoPE 0.25 不变。48 层中占 12 层，而且每一层同时也是 [QSA](./qsa.zh.md) 层，因此这里 GQA 与内容相关稀疏是叠加关系而非互斥。                                                                                                                                                                                                                                                                                                             |
