...
- **DeepSeek-V4.1** 放弃了命名的文字档位，改为*连续整数*（1–100），以 `<｜System｜>` 前缀渲染（"Reasoning Effort: 75 (range 1-100, …)"）；`low` / `high` / `max` 只作为 50 / 75 / 100 的别名保留。

四者都不是控制 token、不是路由决策、也不是另一套权重。所有情况下档位都只是模型读到的文本，这就给各家提出了同一个开放问题：策略到底有没有针对这些字符串做过 RL 训练，还是说这个枚举只是厂商钦定措辞的 prompt engineering？目前已有两家公开了训练方法：Kimi K3 在按题目设定的 token 预算奖励下分别训练各强度档的专家模型，再蒸馏回单个 checkpoint；DeepSeek-V4.1 训练单一策略——每个 prompt 在多个强度档上采样，在每个（prompt, 强度）子组内计算优势，并施加系数随强度指数衰减的长度惩罚——这也是它能在训练档位之间插值的原因。其余实现仍没有公开记录。
...
- 原始论文：—（没有正式发表）
- 参考实现：各模型仓库的 chat template——Qwen3.8 / Kimi K3 看 `tokenizer_config.json`；DeepSeek-V4-Flash-0731 和 DeepSeek-V4.1-Flash 看 `encoding/README.md`。
- 相关博客 / 文章：—
...
| DeepSeek-V4.1-Flash    | **第四种机制：连续的整数标量。** `reasoning_effort` ∈ [1, 100]，只在对话开头以 `<｜System｜>` 块渲染一次——`Reasoning Effort: {budget} (range 1-100, the higher the value, the more thorough the reasoning)`。别名 `low` / `high` / `max` = 50 / 75 / 100；默认 **`high`**（0731 的默认档 `low` 没有前缀）。训练方式有公开记录：每个 prompt 在多个强度档上采样回复，优势在每个（prompt, 强度）子组内做均值中心化，长度惩罚系数随强度指数衰减，`k(b) = k0·exp(−(b − b_min)/τ)`。只在有限个档位上训练，中间值靠插值。强度从 25 提到 100，DeepSWE v1.1 从 66.0 升到 74.2，输出 token 约 2.5×。 |
