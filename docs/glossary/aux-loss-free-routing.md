...
| Kimi K3                                  | Keeps the scheme but replaces the **bias-update rule** with **Quantile Balancing**: the fixed-step `b ← b + γ·sign(mean_load − load)` trades slow adaptation against oscillation, and neither setting behaves at ~10³ experts per layer. QB sets each expert's bias directly from the router-score quantile matching its target load `q = mk/n`; Top-(k+1) routing supplies each token's cutoff for free, and the global-batch quantile is estimated from a per-expert histogram reduced by one all-reduce of bin counts. Bias still affects dispatch only, applies next step, and is frozen at inference. See [Stable LatentMoE](./latentmoe.md). |
| DeepSeek-V3.2-Exp                        | `topk_method='noaux_tc'` with sigmoid affinity, unchanged from DeepSeek-V3 — one of the many things held constant so DSA is the only variable under test. Bias-update speed and sequence-balance-loss weight are not restated in the V3.2 paper.                                                                                                                                                                                                                                                                                                                                                                                                   |
| GLM-5.2                                  | `topk_method='noaux_tc'` with sigmoid scoring, 256 experts top-8 + 1 shared, `routed_scaling_factor=2.5` — unchanged from GLM-5 / 5.1. New: `moe_router_dtype='float32'` pins the router to fp32 regardless of the bf16 model dtype.                                                                                                                                                                                                                                                                                                                                                                                                               |
